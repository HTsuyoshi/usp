...
/* Imprimir as sub-chaves */

static void print_key_E_D(int i, subkey_t ks, ubyte left[], ubyte right[]) {
    int j ;
    printf("Subkey %d  : 0x", i);
    for (j = 0; j<6; j++)
        printf("%02x", ks[i][j]);
    printf("\n");

    printf("(E%d, D%d): (0x", i, i);
    for (j = 0; j<4; j++)
        printf("%02x", left[j]);
    printf("; 0x");
    for (j = 0; j<4; j++)
...
static void processMessage(const ubyte *message, subkey_t ks, ubyte *ep) {
    ...
    for (i = 1; i < 17; ++i) {
        ...
        /* print key and (Ek,Dk) */
        print_key_E_D(i, ks, left[i], right[i]);
    }
    printf("\n");
...
void print_tarefa(char* frase) {
    char* contorno = "**********";
    printf("%s %s %s\n\n", contorno, frase, contorno);
}

int bit_diff(const ubyte *ED, const ubyte *EDc) {
    int i, diff = 0;
    for(i=0; i<8*8; i++) {
        if (peekBit(ED,i) != peekBit(EDc, i))
            diff++;
        /* printf("ED %d, EDc %d\n", peekBit(ED,i), peekBit(EDc,i)); */ 
    }
...
int main() {
    const key_t keys[] = {
        /* Chave */
        {0x02, 0x09, 0x00, 0x07, 0x00, 0x01, 0x00, 0x00},
        /* Chave complementada */
        {0x82, 0x09, 0x00, 0x07, 0x00, 0x01, 0x00, 0x00} 
    };
    int len;

    const ubyte nrousp[] = { 0x11, 0x79, 0x60, 0x22, 0x11, 0x79, 0x60, 0x22 };
    /*
    const ubyte nrouspcomp[] = { 0x91, 0x79, 0x60, 0x22, 0x11, 0x79, 0x60, 0x22 };
    */

    /* Mensagem complementada */
    /*
    print_tarefa("Mensagem normal");
    len = sizeof(nrousp) / sizeof(ubyte);
    driver(keys[0], nrousp, len);

    print_tarefa("Mensagem complementada");
    len = sizeof(nrousp) / sizeof(ubyte);
    driver(keys[0], nrouspcomp, len);
    */

    /* Chave complementada */
    print_tarefa("Chave normal");
    len = sizeof(nrousp) / sizeof(ubyte);
    driver(keys[0], nrousp, len);

    print_tarefa("Chave complementada");
    len = sizeof(nrousp) / sizeof(ubyte);
    driver(keys[1], nrousp, len);

    /*
    const ubyte E1D1[] = {0x00, 0xee, 0x22, 0x88, 0x32, 0xa6, 0xf7, 0x3e};
    const ubyte E1cD1c[] = {0x01, 0xee, 0x22, 0x88, 0x76, 0xaf, 0xf6, 0x2e};

    printf("O numero de bits diferentes entre E1D1 e E1cD1c: %d\n", bit_diff(E1D1, E1cD1c));

    const ubyte E2D2[] = {0x32, 0xa6, 0xf7, 0x3e, 0x63, 0xc3, 0x0f, 0x2a};
    const ubyte E2cD2c[] = {0x76, 0xaf, 0xf6, 0x2e, 0xbd, 0x7b, 0x26, 0xf0};

    printf("O numero de bits diferentes entre E2D2 e E2cD2c: %d\n", bit_diff(E2D2, E2cD2c));
    */
