...
*
* Reference: https://cs.ru.nl/~joan/papers/JDA_VRI_Rijndael_2002.pdf
*/
...
void test_aes_with_multiple_key_and_block_lengths(int KC, int BC) {
    int i, j;
    word8 a[4][MAXBC], rk[MAXROUNDS + 1][4][MAXBC], sk[4][MAXKC];

    ROUNDS = numrounds[KC - 4][BC - 4];

    /* Sets block to be encrypted as 0xAA 0xAA ........ */
    for (j = 0; j < BC; j++)
        for (i = 0; i < 4; i++) a[i][j] = 0xAA;
    ...
    /* Sets encryption key to 12345 ... = 0x01 0x02 0x03 ... */
    for (j = 0; j < KC; j++)
        for (i = 0; i < 4; i++) sk[i][j] = i + j*4;

    printf("Encryption key:   ");
    for (j = 0; j < KC; j++)
        for (i = 0; i < 4; i++) printf("%02X", sk[i][j]);
    printf("\n");
...
int main() {
    for (KC = 4; KC <= 8; KC++)
        for (BC = 4; BC <= 8; BC++) {
            test_aes_with_multiple_key_and_block_lengths(KC, BC);
        }
    return 0;
}
